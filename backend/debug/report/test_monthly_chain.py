...
def test_monthly_chain():
    """월간 보고서 생성 테스트"""
    print("=" * 60)
    print("📊 Monthly Report Chain 테스트 시작")
    print("=" * 60)
    
    # 테스트 파라미터
    owner = "김보험"
    target_date = date(2025, 1, 20)  # 2025년 1월 (아무 날짜)
    ...
    try:
        ...
        # 3. 저장된 데이터 확인
        print(f"\n🔍 저장된 보고서 조회...")
        saved_reports = MonthlyReportRepository.list_by_owner(db, owner, limit=5)
        print(f"✅ {owner}의 월간 보고서: {len(saved_reports)}개")
